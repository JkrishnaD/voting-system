...
const ANCOHR_DISCRIMINENT_SIZE: usize = 8;
...
#[derive(Accounts)]
#[instruction(poll_id:u64,)] // instruction is used to pull the different parameters
pub struct InitializePoll<'info> {
    ...
    #[account(
        init,
        payer = user,
        space = ANCOHR_DISCRIMINENT_SIZE + Poll::INIT_SPACE,
...
pub struct InitializeCandidate<'info> {
    ...
    #[account(seeds = [poll_id.to_le_bytes().as_ref()], bump)] // just for referencing
    pub poll: Account<'info, Poll>,

    #[account(
        init,
        payer = user,
        space = ANCOHR_DISCRIMINENT_SIZE + Candidate::INIT_SPACE,
